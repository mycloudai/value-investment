...
### 新增
- **首页快速入口卡片补全**：从 4 张扩充至 8 张，新增《公司解析》、《关键人物》、《精选金句》、《AI 对话》四个入口；AI 对话卡片无数字 badge
...
### 新增
- **AI 回复内文章引用可点击跳转**：AI 回复引用文章时生成 Markdown 链接（如 `[1984年致股东信](/shareholder-letters/1984)`），点击即可 SPA 内跳转到对应文章页面
- **AI 思考检索动画**：工具调用期间显示弹跳圆点动画 + "正在思考检索" 提示，替代之前的静态文字
- **聊天气泡 Markdown 美化**：完整支持标题、列表、引用、代码块、表格、加粗斜体等样式渲染
- **API 调用失败自动重试**：429/500/502/503/529 错误码自动重试最多 2 次，含指数退避

### 修复
- **"工具调用轮次超过上限" 错误**：将安全上限从 5 轮提高到 20 轮，且即使超限也会强制生成最终回答而非报错
- **参考原文面板不再显示无用的 "搜索中..."**：思考期间不再在参考面板写入占位条目，仅在获得真实结果后更新
- **参考原文跨轮累积**：多轮工具调用的检索结果合并显示在参考面板中，最终答案后再发一次完整引用列表

### 变更
- AI 引用格式从 `【来源：...】` 改为 Markdown 链接格式，检索结果包含文章路由信息供 AI 生成精确链接
